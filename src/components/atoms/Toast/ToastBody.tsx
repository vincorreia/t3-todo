...
export const ToastBody: React.FC<Props> = ({ type, message }) => {
  ...
  return (
    <>
      <div
        className={`flex items-center gap-x-4 lg:gap-x-8 px-8 py-2 lg:py-4 text-white ${colors[type]}`}
      >
        <FontAwesomeIcon
          icon={icon[type]}
          spin={type === "loading"}
        ...
        <div className="flex flex-col gap-y-2 lg:gap-y-4">
          <p className="lg:text-2xl text-xl font-bold capitalize">{type}</p>
          <p className="lg:text-xl font-semibold">{message}</p>
        </div>
